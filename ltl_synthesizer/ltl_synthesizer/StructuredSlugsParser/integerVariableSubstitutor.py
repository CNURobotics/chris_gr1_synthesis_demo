...
from .compiler import computeCalculationSubformula
...
def flatten_as_much_as_possible(tree):
    ...
    # TODO: Figure out what is being compared here
    if (type(tree).isinstance(type("A"))) or (type(tree).isinstance(type(u'A'))):
        return tree
    newTree = []
    for a in tree:
...
def translateTreeToStructuredSlugsLine(tree, isPrimed):
    ...
    if (tree[0] == "Conjunction"):
        assert len(tree) > 1
        return "("+"&".join(["("+translateTreeToStructuredSlugsLine(a, isPrimed)+")" for
                             a in tree[1:]])+")"
    if (tree[0] == "Disjunction"):
        assert len(tree) > 1
        return "("+"|".join(["("+translateTreeToStructuredSlugsLine(a, isPrimed)+")" for
    ...
    if (tree[0] == "Assignment"):
        ...
        if isPrimed:
            if "'" in var:
                raise Exception("Cannot parse input formula: variable is both primed"
                                "and in the scope of a next-operator")
            var = var + "'"
        return var
    if (tree[0] == "TRUE"):
        return "TRUE"
    ...
    if (tree[0] == "CalculationSubformula"):
        ...
        if operation == "GreaterOperator":
            ...
            print("Error: Could not interpret calculation comparison operation: " + operation,
                  file=sys.stderr)
            raise 1
    ...
    if (tree[0] == "numID"):
        if (isPrimed):
            print("Error: This tool does not allow the use of a next-time operator in combination"
                  " with integer variable expressions.", file=sys.stderr)
    ...
    for i in range(len(tokens)-1, -1, -1):
        ...
        if currentToken == "|" or currentToken == "&" or currentToken == "^":
            ...
            if currentToken in booleanAPs:
                ...
                print('failed in integerVariableSubstitutor')
...
# -----------------------------------------------------------------------------------------------
# Recursive Worker function to replace some variable by other expressions,
# possibly with some counter-part
# -----------------------------------------------------------------------------------------------
def treeReplaceExpression(tree, replacementData):
    ...
    if tree[0] == "numID":
        if tree[1] in replacementData:
            ...
            return (tree, [])
    if (tree[0] == "NumberBrackets" or tree[0] == "AtomicNumberExpression" or
            tree[0] == "NumberExpression"):
        # Bracketed expressions are fine as long as there is no multiplications,
    ...
    if tree[0] == "MultiplicationOperator" or tree[0] == "NegationOperator":
        raise ("Error: Multiplications and Negations are not allowed"
               " in the integerVariableSubstitutor")
    ...
    print("Error: Could not apply treeReplaceExpression to expression of the type " + str(tree[0]),
          file=sys.stderr)
    printTree(tree)
    raise "Failed"
...
def treeReplace(tree, replacementData):
    if tree[0] == "CalculationSubformula":
        ...
        if len(toBeAdded2) == 0:
            ...
            translatedSubtree1 = ("NumberExpression", newSubtree1, ("AdditionOperator",),
                                  ("numID", "+".join(toBeAdded2)))
        if len(toBeAdded1) == 0:
            translatedSubtree2 = newSubtree2
        else:
            translatedSubtree2 = ("NumberExpression", newSubtree2, ("AdditionOperator",),
...
if __name__ == '__main__':
    if len(sys.argv) < 3:
        print('Error: Need input file and replacement file as parameters', file=sys.stderr)
        sys.exit(1)
