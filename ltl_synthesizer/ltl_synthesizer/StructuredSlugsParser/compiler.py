#!/usr/bin/python
#
# Translates a structured specification into an unstructured one that
# is suitable to be read by the slugs synthesis tool
...
def flatten_as_much_as_possible(tree):
    ...
    if len(tree) == 1:
        return tree
    # TODO: How to do this in the way intended?
    # TODO: Figure out intended type comparisons
    if ((type(tree) == (type("A"))) or (type(tree) == (type(u"A")))):
        return tree
    newTree = []
    for a in tree:
...
def parseLTL(ltlTxt, reasonForNotBeingASlugsFormula):
    ...
    try:
        ...
        for t, e in exception.errors:
            ...
            print("... which could not have been a slugs Polish notation line because of: "
                  + reasonForNotBeingASlugsFormula, file=sys.stderr)
            print("Could not parse {}, ".format(found), file=sys.stderr)
            print("Wanted a token of one of the following forms: "+", ".join([repr(s) for s in e]),
                  file=sys.stderr)
        raise
...
def recurseCalculationSubformula(tree, memoryStructureParts, isPrimed):
        ...
        if tree[2][0] == "AdditionOperator":
            ...
            if len(part2MemoryStructurePointers) < len(part1MemoryStructurePointers):
                part1MemoryStructurePointers, part2MemoryStructurePointers = (
                    part2MemoryStructurePointers, part1MemoryStructurePointers)

            startingPosition = len(memoryStructureParts)

            memoryStructureParts.append(["^", part1MemoryStructurePointers[0],
                                         part2MemoryStructurePointers[0]])
            ...
            for i in range(1, len(part1MemoryStructurePointers)):
                memoryStructureParts.append(["^ ^", part1MemoryStructurePointers[i],
            ...
            for i in range(len(part1MemoryStructurePointers), len(part2MemoryStructurePointers)):
                memoryStructureParts.append(["^", part2MemoryStructurePointers[i], "?",
                                             str(len(memoryStructureParts)-1)])
                memoryStructureParts.append(["&", part2MemoryStructurePointers[i], "?",
                                             str(len(memoryStructureParts)-2)])

            # Return new value including the final bit
            return (["? "+str(i*2+startingPosition)
    ...
    # Work on NumID
        ...
        if primedLocally and isPrimed:
            raise Exception("Variable is used primed in the scope of a next-operator,"
                            " which is not supported: "+apName)
        if primedLocally:
    ...
    # Overwrite the least significant bits of some expression
        ...
        part1MemoryStructurePointers = recurseCalculationSubformula(tree[1],
                                                                    memoryStructureParts, isPrimed)
        part2MemoryStructurePointers = recurseCalculationSubformula(tree[3],
                                                                    memoryStructureParts, isPrimed)
        assert len(part1MemoryStructurePointers) < len(part2MemoryStructurePointers)
        return part1MemoryStructurePointers + part2MemoryStructurePointers[
                                                len(part1MemoryStructurePointers):]
...
def computeCalculationSubformula(tree, isPrimed):
    ...
    part1MemoryStructurePointers = recurseCalculationSubformula(addMinimumValueToAllVariables(
                                    tree[1]), memoryStructureParts, isPrimed)
    part2MemoryStructurePointers = recurseCalculationSubformula(addMinimumValueToAllVariables(
                                    tree[3]), memoryStructureParts, isPrimed)
    print("P1: "+str(part1MemoryStructurePointers), file=sys.stderr)
    print("P2: "+str(part2MemoryStructurePointers), file=sys.stderr)
    print("MSPatComp: "+str(memoryStructureParts), file=sys.stderr)
    ...
    if (tree[2][1][0] == "GreaterOperator") or (tree[2][1][0] == "GreaterEqualOperator"):
        ...
        for i in range(0, min(len(part1MemoryStructurePointers),
                              len(part2MemoryStructurePointers))):

            thisParts = ["|", "&", part1MemoryStructurePointers[i], "!",
                         part2MemoryStructurePointers[i], "& | !",
                         part2MemoryStructurePointers[i],
    ...
    # ->    Smaller or Smaller-Equal
        ...
        for i in range(0, min(len(part1MemoryStructurePointers),
            ...
            thisParts = ["|", "& !", part1MemoryStructurePointers[i],
                         part2MemoryStructurePointers[i], "& |", part2MemoryStructurePointers[i],
                         "!", part1MemoryStructurePointers[i]]+thisParts
        for i in range(len(part2MemoryStructurePointers), len(part1MemoryStructurePointers)):
            thisParts = ["& !", part1MemoryStructurePointers[i]]+thisParts
        for i in range(len(part1MemoryStructurePointers), len(part2MemoryStructurePointers)):
...
def parseSimpleFormula(tree, isPrimed):
    if (tree[0] == "Formula"):
        # print('*'*20, flush=True, file=sys.stderr)
        # print('\nFormula', flush=True, file=sys.stderr)
        assert len(tree) == 2
        return parseSimpleFormula(tree[1], isPrimed)
    if (tree[0] == "Biimplication"):
        # print('\nBiimplication', flush=True, file=sys.stderr)
        # print(tree[1], flush=True, file=sys.stderr)
        # print(tree[2], flush=True, file=sys.stderr)
        b1 = parseSimpleFormula(tree[1], isPrimed)
        b2 = parseSimpleFormula(tree[2], isPrimed)
        # print(b1, flush=True, file=sys.stderr)
        # print(b2, flush=True, file=sys.stderr)
        return ["|", "&", "!"]+b1+["!"]+b2+["&"]+b1+b2
    if (tree[0] == "Implication"):
        # print('\nImplication', flush=True, file=sys.stderr)
        b1 = parseSimpleFormula(tree[1], isPrimed)
        b2 = parseSimpleFormula(tree[2], isPrimed)
        return ["|", "!"]+b1+b2
    if (tree[0] == "Conjunction"):
        # print('\nConjuction', flush=True, file=sys.stderr)
        ret = parseSimpleFormula(tree[1], isPrimed)
        for a in tree[2:]:
            # print('\t', ret, flush=True, file=sys.stderr)
            ret = ["&"]+ret+parseSimpleFormula(a, isPrimed)
        # print('line 465', ret, flush=True, file=sys.stderr)
        return ret
    if (tree[0] == "Disjunction"):
        # print('\nDisjunction', flush=True, file=sys.stderr)
        ret = parseSimpleFormula(tree[1], isPrimed)
        for a in tree[2:]:
            ret = ["|"]+ret+parseSimpleFormula(a, isPrimed)
        return ret
    if (tree[0] == "UnaryFormula"):
        # print('\nUnaryFormula', flush=True, file=sys.stderr)
    ...
    if (tree[0] == "Assignment"):
        ...
        if isPrimed:
            if "'" in var:
                raise Exception("Cannot parse input formula: variable is both primed and"
...
def isValidRecursiveSlugsProperty(tokens):
    ...
    for i in range(len(tokens)-1, -1, -1):
        ...
        if currentToken == "|" or currentToken == "&" or currentToken == "^":
            if stacksize < 2:
                print(f'failed in compiler: Rejected part due to stack underflow with {stacksize}'
                      ' and {tokens}', file=sys.stderr, flush=True)
            ...
            if currentToken in booleanAPs:
                ...
                # print(f'failed in compiler with {stacksize} and {tokens}',
                #       file=sys.stderr, flush=True)
                # print(f"Rejected part '{tokens[i]}' ('{currentToken}') AP={currentToken in booleanAPs} (i={i}) when reading right-to-left.", file=sys.stderr, flush=True)
                # print(f" booleanAP={booleanAPs}", file=sys.stderr, flush=True)
...
def performConversion(inputFile, thoroughly):
    ...
    mode = ""
    lines = {"[ENV_TRANS]": [], "[ENV_INIT]": [], "[INPUT]": [], "[OUTPUT]": [], "[SYS_TRANS]": [],
             "[SYS_INIT]": [], "[ENV_LIVENESS]": [], "[SYS_LIVENESS]": [],
             "[OBSERVABLE_INPUT]": [], "[UNOBSERVABLE_INPUT]": [], "[CONTROLLABLE_INPUT]": []}
    ...
    # ---------------------------------------
    translatedIOLines = {"[INPUT]": [], "[OUTPUT]": [], "[OBSERVABLE_INPUT]": [],
    ...
    for variableType in ["[INPUT]", "[OUTPUT]", "[OBSERVABLE_INPUT]",
        ...
        for line in lines[variableType]:
            ...
            if "'" in line:
                print("Error with atomic signal name " + line + ": the name must not contain any"
            ...
            if ":" in line:
                ...
                if len(parts) != 2:
                    print("Error reading line '" + line + "' in section " + variableType +
                          ": Too many ':'s!", file=sys.stderr)
                ...
                if minValue > maxValue:
                    print("Error reading line '" + line + "' in section " + variableType +
                          ": the minimal value should be smaller than the maximum one "
                          "(or at least equal)", file=sys.stderr)
                    raise Exception("Failed to translate file.")
                ...
                # Translate name into bits in a way such that the first bit carries
                # not only the original name, but also min and max information
                translatedNames[parts[0]] = [parts[0] + "@" + str(i) + ("." + str(minValue)
                                                                        + "." + str(maxValue)
                                                                        if i == 0 else "")
                                             for i in range(0, nofBits)]

                translatedIOLines[variableType] = (translatedIOLines[variableType]
                                                   + translatedNames[parts[0]])

                # print(f"extending booleanAP with '{translatedNames[parts[0]]}'",
                ...
                if (2**numberAPNofBits[variable]) != limitDiff:
                    ...
                    for i in range(0, numberAPNofBits[variable]):
                        if 2**i & limitDiff:
                            ...
                            tokens = ["& !", translatedNames[variable][i]]+tokens
                    lines["["+propertyDestination+"_INIT]"].append("## Variable limits: "
                                                                   + str(minValue) + "<="+variable
                                                                   + "<=" + str(maxValue))
                    lines["["+propertyDestination+"_INIT]"].append(" ".join(tokens))

                    # Transition constraint for previous state -- only if using the
                    # "--thorougly mode"
                    if thoroughly:
                        lines["["+propertyDestination+"_TRANS]"].append("## Variable limits: "
                                                                        + str(minValue) + "<="
                                                                        + variable + "<="
                                                                        + str(maxValue))
                ...
                line = line.strip()
                # print(f"adding normal AP <{line}>", flush=True, file=sys.stderr)
                booleanAPs.append(line)
                booleanAPs.append(line+"'")
                # print(f"   booleanAPs={booleanAPs}", flush=True, file=sys.stderr)
                translatedIOLines[variableType].append(line)

    # ---------------------------------------
    # Output new input/output lines
    # ---------------------------------------
    for variableType in ["[INPUT]", "[OUTPUT]", "[OBSERVABLE_INPUT]", "[UNOBSERVABLE_INPUT]",
                         "[CONTROLLABLE_INPUT]"]:
    ...
    # ---------------------------------------
    for propertyType in ["[ENV_TRANS]", "[ENV_INIT]", "[SYS_TRANS]", "[SYS_INIT]",
        ...
        if len(lines[propertyType]) > 0:
            ...
            for a in lines[propertyType]:
                cnt += 1
                # print(f"{cnt:4d}  >{propertyType}: >{a.strip()}<", flush=True, file=sys.stderr)
                if a.strip()[0:1] == "#":
                    print(a)
                else:
...
if __name__ == "__main__":
    ...
    print(translatedNames, file=sys.stderr)
