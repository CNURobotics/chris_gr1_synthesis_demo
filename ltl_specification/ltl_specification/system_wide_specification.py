...
file_path = os.getenv('FLEXBE_SYNTHESIS_ADDITIONAL_SPECS')
...
class CompleteSpecification(GR1Specification):
    ...
    #     # Now generate LTL formulas encoding all of the initial conditions
        ...
        if fpath is not None:
            print(fpath, flush=True, file=sys.stdout)
            loaded_specs = SpecsLoader(fpath)
            self.merge_gr1_specifications([loaded_specs])

        self.write_structured_slugs_file(os.path.dirname(file_path))
    ...
    def convert_structured_slugs_to_slugsin(self, name):
        ...
        file_dir = os.path.join(os.path.dirname(file_path), name, name)
        print(file_dir)
        slugsin_file = file_dir + '.slugsin'
