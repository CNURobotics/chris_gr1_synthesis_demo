...
class GR1Formula(object):
    ...
    def _add_props_from_ts(self):
        '''
        Reads the items in the TS dictionary,
        adds them to the system propositions, if they are not already there.
        '''
        my_props = list(set(list(self.sys_props)))
        self.sys_props = my_props
        props_to_add = list(self.ts.keys())
        modified_copy = list()
        for prop in props_to_add:
            modified_copy.append(prop + '_a')  # TODO not ideal in the long term
        for v in self.ts.values():
            for prop in v:
                modified_copy.append(prop + '_a')  # TODO not ideal in the long term

        self.sys_props = list(set(list(self.sys_props) + modified_copy))

    def _gen_phi_prop(self, prop):
        '''
        Generate (non-atomic) proposition of the form \phi_r,  # noqa: W605
    ...
    def _get_other_trans_props(self, prop):
        '''
        For some proposition \pi_r, get all propositions \pi_r'
        such that r' =/= r.  # noqa: W605
        '''
        ...
        if prop in self.sys_props:
            ...
            raise ValueError('gr1_formulas. _get_other_trans_props - Unknown type for proposition: '
                             '{}'.format(prop))
