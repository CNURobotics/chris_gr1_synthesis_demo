...
from .gr1_formulas import GR1Formula, SimpleLivenessRequirementFormula
...
class ActivationOutcomesFormula(GR1Formula):
    ...
    def __init__(self, sys_props, outcomes=['completed'], ts=dict()):
        ...
        try:
            ...
            print("Exception in ActivationOutcomeFormulas init: {exc}")
            raise exc

        self.outcomes = outcomes
        sys_props = sys_props + list(ts.keys())  # FIXME
    ...
    def _check_input_arguments(self, sys_props, outcomes, ts):
        ...
        if len(ts) > 0:
            ...
            # print("  ts dictionary is empty!", flush=True)
            pass
    ...
    def _get_env_props_from_outcome_props(self):
        ...
        env_props = reduce(lambda acc, ele: acc + ele,
                           self.outcome_props.values())  # TODO fix reduce call
        return env_props
...
class OutcomeMutexFormula(ActivationOutcomesFormula):
    ...
    def __init__(self, sys_props, outcomes):
        super().__init__(sys_props=sys_props, outcomes=outcomes)
...
class TransitionRelationFormula(ActivationOutcomesFormula):
    ...
    def __init__(self, ts):
        super(TransitionRelationFormula, self).__init__(sys_props=[], ts=ts)  # FIXME Causes Crash
...
def _get_act_nothing(props):
    ...
    mapped = map(_get_act_prop, props)
    negged = map(LTL.neg, mapped)
    conjd = LTL.conj(negged)
    return conjd
