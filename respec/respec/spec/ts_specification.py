...
from ..formula.activation_outcomes import (
    OutcomeMutexFormula,
    ...
    def _gen_act_out_topology_formulas(self, outcomes):
        try:
            ...
            constraints_formula = TopologyOutcomeConstraintFormula(
                                                ts=self.ts,
                                                outcomes=outcomes)
            mutex_formula = OutcomeMutexFormula(sys_props=list(self.ts.keys()),
                                                outcomes=outcomes)
            deactivation_formula = PropositionDeactivationFormula(
                                                sys_props=list(self.ts.keys()),
                                                outcomes=outcomes)
            ...
            print(f"Exception in ts_specification: {exc}", flush=True)
            raise exc
